// Deno-compatible copy — identical logic to src/lib/scheduling-engine/quality-score.ts

import type { ShiftInput, RuleViolation, QualityMetrics, ScheduleContext } from "./types.ts";
import { getDayOfWeek, getWeekDates, shiftDuration } from "./hard-rules.ts";
...
export function calculateMetrics(
  ...
  const coverageRespectedPct = Math.max(0, Math.round(((estimatedCoverageChecks - coverageViolations) / estimatedCoverageChecks) * 100));
  const contractViolations = hardViolations.filter(v => v.ruleId === "HR002").length;
  const contractHoursRespectedPct = Math.max(0, Math.round(((empCount - contractViolations) / empCount) * 100));
  ...
  const approvedRequestsRespectedPct = Math.max(0, Math.round(((totalRequests - timeOffViolations) / totalRequests) * 100));
  ...
  const preferencesRespectedPct = Math.max(0, Math.round(((empsWithPrefs - Math.min(prefWarnings, empsWithPrefs)) / empsWithPrefs) * 100));
  ...
  return { coverageRespectedPct, contractHoursRespectedPct, approvedRequestsRespectedPct, preferencesRespectedPct, weekendEquityPct, openCloseEquityPct, hourDeviationPerEmployee, hardViolationCount: hardViolations.length, softWarningCount: softWarnings.length };
}
