...
/** Parses "HH:MM" or "HH:MM:SS" to fractional hours (e.g. "08:30" → 8.5). */
...
/** Work-shift duration in hours; 0 for day-off or incomplete shifts. */
export function shiftDuration(shift: ShiftInput): number {
  ...
  if (e === 0) e = 24; // midnight expressed as "00:00"
  if (e < s) e += 24;  // cross-midnight shift (e.g. 22:00–02:00 → 22–26)
  return Math.max(0, e - s);
}

/** Day-of-week from "YYYY-MM-DD": 0=Mon … 6=Sun. */
export function getDayOfWeek(date: string): number {
  const d = new Date(date + "T00:00:00Z");
  return (d.getUTCDay() + 6) % 7;
}

/** All dates (inclusive) between weekStart and weekEnd. */
...
/** True when a work shift spans the given hour slot. */
...
// ─── HR001 — Minimum (and maximum) coverage per slot ────────────────────────
...
export function checkMinimumCoverage(
  ...
  for (const req of context.coverageRequirements) {
    ...
    for (const date of matchingDates) {
      if (holidays.has(date)) continue; // handled by HR012
      ...
      // Only check max when explicitly configured (undefined/null = no upper limit)
...
// ─── HR002 — Contract hours within ±tolerance ────────────────────────────────
...
// ─── HR003–HR009 — Approved time-off blocks ──────────────────────────────────

const FULL_DAY_BLOCK_TYPES = new Set([
  "ferie", "permesso", "permesso_104", "malattia", "giorno_libero",
]);
...
const LABEL_MAP: Record<string, string> = {
  ferie: "ferie approvate",
  permesso: "permesso approvato",
  permesso_104: "permesso 104 approvato",
  malattia: "malattia validata",
  giorno_libero: "giorno libero approvato",
};

export function checkTimeOffBlocks(
  shifts: ShiftInput[],
  context: ScheduleContext,
): RuleViolation[] {
  const violations: RuleViolation[] = [];
  ...
  for (const toff of context.approvedTimeOff) {
    const workShifts = shifts.filter(
      s => s.userId === toff.userId && s.date === toff.date && !s.isDayOff,
    );
      ...
      for (const s of workShifts) {
        if (s.startTime && parseHours(s.startTime) < morning) {
          violations.push({
            ruleId: "HR008",
            severity: "hard",
            description: `Turno mattutino (${s.startTime}) assegnato con mattina libera approvata il ${toff.date}`,
            affectedEmployeeId: toff.userId,
            affectedDate: toff.date,
            details: { startTime: s.startTime, morningCutoff: morning },
          });
        }
      }
    }
      ...
      for (const s of workShifts) {
        if (s.startTime && parseHours(s.startTime) >= evening) {
          violations.push({
            ruleId: "HR009",
            severity: "hard",
            description: `Turno serale (${s.startTime}) assegnato con sera libera approvata il ${toff.date}`,
            affectedEmployeeId: toff.userId,
            affectedDate: toff.date,
            details: { startTime: s.startTime, eveningStart: evening },
          });
        }
      }
    }
...
// ─── HR010 — No overlapping shifts per employee ──────────────────────────────
...
export function checkNoOverlaps(shifts: ShiftInput[]): RuleViolation[] {
  ...
  for (const [, dayShifts] of byKey) {
    ...
    const sorted = [...dayShifts].sort(
      (a, b) => parseHours(a.startTime!) - parseHours(b.startTime!),
    );
    for (let i = 1; i < sorted.length; i++) {
      ...
      if (parseHours(curr.startTime!) < prevEnd) {
        violations.push({
          ruleId: "HR010",
          severity: "hard",
          description: `Turni sovrapposti il ${curr.date}: ${prev.startTime}–${prev.endTime} e ${curr.startTime}–${curr.endTime}`,
          affectedEmployeeId: curr.userId,
          affectedDate: curr.date,
          details: {
            shift1: `${prev.startTime}–${prev.endTime}`,
            shift2: `${curr.startTime}–${curr.endTime}`,
          },
        });
      }
    }
...
export function checkMinShiftDuration(
  shifts: ShiftInput[],
  context: ScheduleContext,
): RuleViolation[] {
  ...
  for (const s of shifts) {
    ...
    if (dur < minH) {
      violations.push({
        ruleId: "HR011",
        severity: "hard",
        description: `Turno troppo corto: ${dur.toFixed(1)}h (minimo ${minH}h) il ${s.date}`,
        affectedEmployeeId: s.userId,
        affectedDate: s.date,
        details: { durationH: dur, minShiftHours: minH, startTime: s.startTime, endTime: s.endTime },
      });
    }
  }
...
// ─── HR012 — Holiday coverage +20% ──────────────────────────────────────────

export function checkHolidayCoverage(
  shifts: ShiftInput[],
  context: ScheduleContext,
): RuleViolation[] {
  if (!context.holidays || context.holidays.length === 0) return [];
  const violations: RuleViolation[] = [];
  ...
  for (const holiday of context.holidays) {
    ...
    const reqs = context.coverageRequirements.filter(r => r.dayOfWeek === dow);

    for (const req of reqs) {
      const slotH = parseHours(req.hourSlot);
      const requiredWithBonus = Math.ceil(req.minStaffRequired * multiplier);
      const count = shifts.filter(
        s => s.date === holiday && s.department === req.department && coversHour(s, slotH),
      ).length;
      ...
      if (count < requiredWithBonus) {
        violations.push({
          ruleId: "HR012",
          severity: "hard",
          description: `Copertura festiva insufficiente: ${count}/${requiredWithBonus} in ${req.department} il ${holiday} alle ${req.hourSlot} (+${Math.round((multiplier - 1) * 100)}%)`,
          affectedEmployeeId: "",
          affectedDate: holiday,
          details: {
            department: req.department,
            hourSlot: req.hourSlot,
            baseRequired: req.minStaffRequired,
            requiredWithBonus,
            actual: count,
            multiplier,
          },
...
export function runAllHardRules(
  shifts: ShiftInput[],
  context: ScheduleContext,
): RuleViolation[] {
  return [
